...
public class MiniMaxGameImproved {
    ...
    public long miniMax(GameNode node) {
        if (depthLimit == node.mDepth || node.isLeaf()) {
            numPositionsEval++;
            node.mValue = StaticEstimationImp1.getMidEndGameEstImproved(node);
            return node.mValue;
        }
    ...
    public static void main(String[] args) {
        ...
        miniMaxGameImproved.depthLimit = Integer.parseInt(args[2]);
        miniMaxGameImproved.numPositionsEval = 0;
        long est = miniMaxGameImproved.miniMax(node);

        IOHelper.cout(node.mBestMove.mPositon, miniMaxGameImproved.numPositionsEval, est);
